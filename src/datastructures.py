...
class FamilyStructure:
    ...
    def update_member(self, id, updatedMember):
        # fill this method and update the return
        if updatedMember["first_name"] is None or updatedMember["age"] is None or updatedMember["lucky_numbers"] is None:
            return "You must include 'first_name', 'age' and 'lucky_numbers'", 400

        elif type(updatedMember["lucky_numbers"]) is not list:
            return "'lucky_numbers' must be a list of numbers", 400
            ...
            try:
                ...
                for i, member in enumerate(self._members):
                    if member["id"] == int(id):
                        updatedMember["id"] = id
                        updatedMember["last_name"] = self.last_name

                        self._members[i] = updatedMember
                        return updatedMember, 200
